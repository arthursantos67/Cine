...
def on_task_prerun(task_id=None, task=None, *args, **kwargs):
    ...
    if not hasattr(_task_locals, "tokens"):
        _task_locals.tokens = {}
    _task_locals.tokens[task_id] = (correlation_token, execution_token)
...
def on_task_postrun(task_id=None, task=None, state=None, *args, **kwargs):
    ...
    tokens = getattr(_task_locals, "tokens", {}).pop(task_id, None)
    if tokens:
